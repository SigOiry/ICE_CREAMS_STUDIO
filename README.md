...
- download the Windows installer directly here:
  [`ICE_CREAMS_Installer_1_0_23.exe`](https://github.com/SigOiry/ICE_CREAMS_STUDIO/raw/main/templates/Output/ICE_CREAMS_Installer_1_0_23.exe)
...
- Added an optional **Spectral 1D CNN** training path alongside the
  legacy tabular dense network
- Spectral 1D CNN training can now include either raw reflectance only
  or raw plus standardized reflectance as saved model metadata
- Apply and Validation now auto-detect the exported model family,
  feature mode, and spectral input layout
- Added post-processing to reclassify output **Class 4** pixels with
  `NDVI < 0.25` to **Class 5**, with `SPC` forced to `-1`
